...
```bash
/store/store5/software/simple-gpu-schedule/with-gpu any --idle-seconds 5 -- \
  python scripts/profile_gpu_action_kernel.py --device cuda --cells 2500 --height 60 --width 80 --families 1 --family-capacity 10 --initial-health 15 --health-dtype int32 --matmul-precision high --compile-mode default --compiled-block-steps 50 --profile-blocks 1 --output-json test_cases/artifacts/gpu_mutation/profile_block_cap10_block50.json
```
...
- The active mutation setup is `shared_rank1_factored`. Each spawn wave gets one shared family base created from the survival-weighted update of round participants; survivors keep their current genome unchanged. Newly spawned cells in that family each get independent rank-1 factors and scalar coefficients around the shared base.
- The factored GPU path applies per-cell rank-1 corrections as `x @ W_base.T + coeff * (x @ v) * u`, so it avoids materializing dense per-cell matrices. Shared-family cells use the same base matrix, while each cell owns its own rank-1 `u`/`v` factors and coefficients. The CUDA planner packs multiple base families into one tensor pass by indexing the right base tensors per cell, which avoids many tiny CUDA launches when later rounds contain small survivor families.
...
- The tensor runtime now treats `--tensor-family-capacity` as a starting cache size rather than a hard gameplay cap: dead family slots are reused, and rare all-live overflow grows the family tensors and recaptures the CUDA graph. The default starts at 10 family rows because 7 can trigger an expensive graph recompile on high-survival normal-play seeds, while 10 avoided timed graph captures in seed 7/8/9 eight-round checks on the RTX A4500.
